    ...
    where T : class, IEntity<long>
{
    ...
    {
        entity.EmployeeNumber = _data.Count + 1;
        _data.Add(entity);
    ...
    {
        return await Task.FromResult(_data.SingleOrDefault(x => x.EmployeeNumber == id));
    }
    ...
    {
        var entity = _data.SingleOrDefault(x => x.EmployeeNumber == id);
        if (entity != null)
        {
            _data.Remove(entity);
            return true;
    ...
    {
        var existing = _data.SingleOrDefault(x => x.EmployeeNumber == entity.EmployeeNumber);
        if(existing != null)
        {
